...
def detectObjects_yolov5(img):
    weights = "/home/sohaib/Documents/kodifly/yolov5/runs/train/exp7/weights/best.pt"
    ...
    list_bbox = [
        list(i)
        for i in np.asarray(
            results.pandas().xyxy[0][["xmin", "ymin", "xmax", "ymax"]].values
        )
    ]
...
def detectObjects_yolov3(img, addr_weights, addr_confg, MIN_CONFIDENCE=0.5):
    ...
    blob = cv2.dnn.blobFromImage(
        img, 1 / 255.0, (416, 416), (0, 0, 0), swapRB=False, crop=False
    )
...
def detectObjects_yolov5(img, addr_weights="", addr_confg="", MIN_CONFIDENCE=0.5):
    ...
    list_bbox = [
        xywh_to_x1y1x3y2(i)
        for i in np.asarray(
            results.pandas().xywh[0][["xcenter", "ycenter", "width", "height"]].values
        )
    ]
